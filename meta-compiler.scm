...
;;This is the modified version of the original file
...
;Problem 2
;finished the compile-application and compile-lambda code
...
(define (compile-call/cc exp env-names)
  (let ((body (compile exp env-names)))
    (lambda (env-values cont)
      (body env-values (lambda (f) (f (list (lambda (x k) (apply cont x))) cont)))))
  )
...
;Problem 1
;(define sample ’((x y z)
;                 (bob carol ted alice) (s p q r)
;                 (cons car cdr null? pair? zero? true false - * + = < 1+)))

(define (lookup-variable v env-names)
  (define (lookup frame val env-names)
    (cond ((null? (car env-names)) (lookup (1+ frame) 0 (cdr env-names)))
          ((eq? v (caar env-names)) (list frame val))
          (else (lookup frame (1+ val) (cons (cdr (car env-names)) (cdr env-names))))))
  (lookup 0 0 env-names))
...
(define ff
  (try '(letrec ((fact
                  (lambda (n) (if (= n 0) 1 (* n (fact (- n 1))))))) fact))
  )

(define ee
  (try '(letrec ((expt
                  (lambda (b e) (if (= e 0) 1 (* b (expt b (- e 1))))))) expt))
  )

(define (link x)
  (lambda w (x w initial-continuation)))
